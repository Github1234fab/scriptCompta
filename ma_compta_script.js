  ...
  if (!compte699Existe || montantNonAttribue === 0) {
    ...
    if (feuilleRapport) {
      ...
      feuilleRapport.getRange('A1:E1').merge()
        .setValue('🎉 TOUTES LES OPÉRATIONS SONT CLASSÉES !')
        .setFontSize(14)
        .setFontWeight('bold')
        .setHorizontalAlignment('center')
...
function assistantCreationRegle(range) {
  ...
  if (nomSheet === 'Rapport Non Attribué') {
    ...
    libelleOriginal = sheet.getRange(row, 2).getValue();
    debit = parseMontant(sheet.getRange(row, 3).getValue());
    credit = parseMontant(sheet.getRange(row, 4).getValue());
  } 
    ...
    debit = parseMontant(sheet.getRange(row, 8).getValue());
    credit = parseMontant(sheet.getRange(row, 9).getValue());
  }
  else {
  ...
  if (suggestionAuto) {
    const confirmation = ui.alert(
      ...
      '• Compte associé : ' + suggestionAuto.compte + ' (' + obtenirLibelleCompte(suggestionAuto.compte) + ')\n\n' +
    ...
    if (confirmation === ui.Button.YES) {
      ...
      // Supprimer visuellement la ligne immédiatement si on est sur la feuille Rapport
      if (nomSheet === 'Rapport Non Attribué') {
        sheet.deleteRow(row);
      } else {
        range.setValue(false); // Décocher la case si c'est Données Brutes
      }
      
      CACHE_REGLES = null;
      SpreadsheetApp.flush(); // Commit la nouvelle règle dans la feuille avant de régénérer
      ...
      genererBalance(true);
      genererTableauSyntheseRegles(true);
      SpreadsheetApp.flush(); // Commit les nouveaux rapports avant de reconstruire le rapport
      genererRapportNonAttribue(true); // Rafraîchir
      
      ui.alert(
        '✅ Règle appliquée',
        'La règle "' + suggestionAuto.motCle + '" ➔ Compte ' + suggestionAuto.compte + ' (' + obtenirLibelleCompte(suggestionAuto.compte) + ') a été ajoutée.\n' +
  ...
  ui.alert(
    '✅ Opération terminée',
    'La règle "' + motCle + '" ➔ Compte ' + numCompte + ' (' + obtenirLibelleCompte(numCompte) + ') a été mémorisée.\n\n' +
  ...
  for (const s of suggestions) {
    for (const mot of s.mots) {
      ...
      if (clean.includes(motNormalise)) {
        let motCleSuggere = mot.toUpperCase();
        return {
          compte: s.compte,
...
/**
 * Récupère le libellé d'un compte depuis le plan comptable
 */
function obtenirLibelleCompte(compte) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const feuillePlan = ss.getSheetByName(CONFIG.FEUILLES.PLAN_COMPTABLE);
  if (feuillePlan) {
    const lastRow = feuillePlan.getLastRow();
    if (lastRow > 1) {
      const data = feuillePlan.getRange(2, 1, lastRow - 1, 2).getValues();
      for (const row of data) {
        if (String(row[0]).trim() === String(compte).trim()) {
          return row[1];
        }
      }
    }
  ...
  // Dictionnaire de secours (fallback)
  const planComptableDict = {
    '512': 'Banque',
    '530': 'Caisse',
    '411': 'Adhérents',
    '467': 'Autres débiteurs',
    '401': 'Fournisseurs',
  ...
  return planComptableDict[compte] || 'Compte inconnu';
}
...
function autoClasserAvecSuggestions() {
  ...
  nouvellesRegles.slice(0, 15).forEach(r => {
    ...
    messageConfirmation += '• "' + r[0] + '" ➔ Compte ' + numCompte + ' (' + obtenirLibelleCompte(numCompte) + ') [' + direction + ']\n';
  });
